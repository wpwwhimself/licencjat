...
\begin{enumerate}
  \item [\textbf{A1}:] $\tr(\doubleC) \coloneqq \int_0^2\doubleC(t,t)\,dt < \oo $
  \item [\textbf{A2}:] $v_1(t) \coloneqq X_1(t) - m(t) $ spełnia własność
  \[
  \doubleE\norm{v_1}^4 = \doubleE \nz{\int_0^2 v^2_1(t)\,dt}^2 <\oo
  \]
  \item [\textbf{A3}:] $\bigwedge_{t\in[0,2]}\nz{ \doubleC(t,t) >0\quad\land\quad \max_{t\in [0,2]} \doubleC(t,t) <\oo} $
  \item [\textbf{A4}:] $\bigwedge_{(s,t)\in [0,2]^2} \doubleE(v_1^2(s)v_1^2(t)) < C < \oo $, gdzie $C$ jest pewną stałą niezależną od $(s,t)$
\end{enumerate}
...
\end{tw}
\begin{proof}
Założenie \textbf{A1} gwarantuje słabą zbieżność (tj. wg rozkładu) funkcji średniej próby do procesu gaussowskiego. Na jego podstawie, jak podaje \cite{Zhang}:
...
\textcolor{blue}{tutaj brakuje opisu testu asymptotycznego}
