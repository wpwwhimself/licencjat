...
\begin{equation}
\label{kow_est}
\hat{\doubleC}(s,t) = \frac{1}{n-1}\sum_{i=1}^n (X_i(s) - \bar{X}(s))(X_i(t) - \bar{X}(t)), \quad s,t\in [0,2]
\end{equation}
...
Równanie \eqref{eqn:zb.as.cn} pozwala na skonstruowanie prostego **algorytmu Monte Carlo** w celu oszacowania końcowej $p$-wartości \cite{MCC}. W tym celu dokonuje się dyskretyzacji danych oraz oszacowania na ich podstawie funkcji kowariancji $\doubleC$, bazując na estymatorze $\hat{\doubleC}$ danym wzorem \eqref{kow_est}. Następnie symulowanych jest $B$ trajektorii na podstawie procesu gaussowskiego ($z(t)$ w Równaniu \eqref{eqn:zb.as.cn}) o macierzy kowariancji $\hat{\doubleC}$. Na podstawie owych trajektorii oblicza się wartość ich statystyk testowych $\tilde{\scriptC_n^i}=\int_0^1 (z_i(t)-z_i(1+t))^2\,dt$, $i=1,2,\dots,B$ oraz końcową $p$-wartość ze wzoru
